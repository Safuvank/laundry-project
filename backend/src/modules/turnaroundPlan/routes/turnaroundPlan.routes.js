 ...
 */
router.get("/admin/all", authenticate, authorize(UserRole.ADMIN), turnaroundPlanController.getAll);
/**
 * Create turnaround plan
 *
 * POST /api/v1/turnaround-plans
 */
router.post("/", authenticate, authorize(UserRole.ADMIN), validateRequest(createTurnaroundPlanSchema), turnaroundPlanController.create);
/**
 * Update turnaround plan
 *
 * PATCH /api/v1/turnaround-plans/:id
 */
router.patch("/:id", authenticate, authorize(UserRole.ADMIN), validateRequest(updateTurnaroundPlanSchema), turnaroundPlanController.update);
/**
 * Activate turnaround plan
 *
 * PATCH /api/v1/turnaround-plans/:id/activate
 */
router.patch("/:id/activate", authenticate, authorize(UserRole.ADMIN), turnaroundPlanController.activate);
/**
 * Deactivate turnaround plan
 *
 * PATCH /api/v1/turnaround-plans/:id/deactivate
 */
router.patch("/:id/deactivate", authenticate, authorize(UserRole.ADMIN), turnaroundPlanController.deactivate);
/*
|--------------------------------------------------------------------------
| Dynamic ID Routes
|--------------------------------------------------------------------------
...
//# sourceMappingURL=turnaroundPlan.routes.js.map