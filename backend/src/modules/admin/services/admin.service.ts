...
class AdminService {
  ...
  async getUsers(query: IAdminUserListQuery) {
  ...
  async updateUser(userId: string, data: IAdminUpdateUser) {
    ...
    if (
      ...
      const existingDeliveryAgent =
        await deliveryAgentRepository.findByUserId(userId);

      if (!existingDeliveryAgent) {
        if (!user.phoneNumber) {
          throw new Error(
            "A phone number is required before promoting the user to a delivery agent.",
          );
        }
  ...
  async getOrders(query: IAdminOrderListQuery) {
    const page = Math.max(1, Number(query.page) || 1);
  ...
  async getDeliveryAgents(query: IAdminDeliveryAgentListQuery) {
    const page = Math.max(1, Number(query.page) || 1);
  ...
  async getDeliveryAgentById(deliveryAgentId: string) {
    if (!mongoose.Types.ObjectId.isValid(deliveryAgentId)) {
      throw new Error("Invalid delivery agent ID.");
    }

    const deliveryAgent =
      await adminRepository.findDeliveryAgentById(deliveryAgentId);
  ...
  async getAssignments(query: IAdminAssignmentListQuery) {
    const page = Math.max(1, Number(query.page) || 1);
  ...
  async getNotifications(query: IAdminNotificationListQuery) {
    const page = Math.max(1, Number(query.page) || 1);
  ...
  async getNotificationById(notificationId: string) {
    ...
    const notification =
      await adminRepository.findNotificationById(notificationId);
  ...
  async getPayments(query: IAdminPaymentListQuery) {
    const page = Math.max(1, Number(query.page) || 1);
