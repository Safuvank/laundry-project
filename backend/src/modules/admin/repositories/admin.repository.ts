...
import type {
  IAdminUpdateUser,
  IAdminUserListQuery,
  IAdminOrderListQuery,
  IAdminDeliveryAgentListQuery,
  IAdminAssignmentListQuery,
  IAdminNotificationListQuery,
  IAdminPaymentListQuery,
...
class AdminRepository {
  ...
  async findUsers(query: IAdminUserListQuery) {
  ...
  async updateUser(userId: string, data: IAdminUpdateUser) {
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      return null;
    }
  ...
  async findDeliveryAgents(query: IAdminDeliveryAgentListQuery) {
    ...
    const [deliveryAgents, total] = await Promise.all([
      DeliveryAgent.find(filter)
        .populate({
          path: "userId",
          select:
            "firstName lastName email phoneNumber role accountStatus isEmailVerified",
        })
        .sort({
          createdAt: -1,
        })
  ...
  async findDeliveryAgentById(deliveryAgentId: string) {
  ...
  async findOrders(query: IAdminOrderListQuery) {
...
async findOrderById(orderId: string) {
  ...
  return Order.findById(orderId)
    ...
    .populate({
      path: "addressId",
      select:
        "_id fullName phoneNumber addressLine1 addressLine2 city state postalCode country addressType isDefault isActive location",
    })
  ...
  async findAssignments(query: IAdminAssignmentListQuery) {
    const {
      page = 1,
      limit = 10,
      search,
      ...
      DeliveryAssignment.find(filter)
        ...
        .populate({
          ...
          populate: {
            path: "userId",
            select:
              "_id firstName lastName email phoneNumber role accountStatus isEmailVerified",
          },
        })
  ...
  async findAssignmentById(assignmentId: string) {
    ...
    return (
      DeliveryAssignment.findById(assignmentId)
        /*
  ...
  async findNotifications(query: IAdminNotificationListQuery) {
  ...
  async findPayments(query: IAdminPaymentListQuery) {
  ...
  async getOrderStatistics() {
    ...
    const completedOrders =
      statusBreakdown.find((item) => item._id === OrderStatus.COMPLETED)
        ?.count ?? 0;

    const cancelledOrders =
      statusBreakdown.find((item) => item._id === OrderStatus.CANCELLED)
        ?.count ?? 0;
  ...
  async getRevenueStatistics(startDate?: Date, endDate?: Date) {
    ...
    const [totalRevenueResult, refundedResult, dailyRevenue] =
      await Promise.all([
        /*
         * Total successful revenue
         */
  ...
  async getPaymentStatistics(startDate?: Date, endDate?: Date) {
    ...
    const [totalPayments, totalAmountResult, statusBreakdown] =
      await Promise.all([
        /*
