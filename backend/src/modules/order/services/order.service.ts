...
import type { IAddress } from "../../address/interfaces/IAddress.js";
import type { ITurnaroundPlan } from "../../turnaroundPlan/interfaces/ITurnaroundPlan.js";
import type { ILaundryService } from "../../laundryService/interfaces/ILaundryService.js";
import type { IPickupSlot } from "../../pickupSlot/interfaces/IPickupSlot.js";
...
class OrderService {
   ...
   */
  private async getOrderOrFail(id: string): Promise<IOrder> {
   ...
   */
  private async validateTurnaroundPlan(
    turnaroundPlanId: string,
  ): Promise<ITurnaroundPlan> {
    this.validateObjectId(turnaroundPlanId, "turnaround plan id");

    const turnaroundPlan =
      await turnaroundPlanRepository.findById(turnaroundPlanId);
   ...
   */
  private async validateLaundryServices(
    laundryServiceIds: string[],
  ): Promise<ILaundryService[]> {
    if (laundryServiceIds.length === 0) {
      throw new ValidationError("Select at least one laundry service.");
    }

    const services: ILaundryService[] = [];
    ...
    for (const serviceId of laundryServiceIds) {
      ...
      if (!service.isActive) {
        throw new ValidationError(
          `Laundry service "${service.name}" is inactive.`,
        );
      }
   ...
   */
  private async validatePickupSlot(pickupSlotId: string): Promise<IPickupSlot> {
  ...
  private async calculateEstimatedPrice(
    ...
    for (const service of services) {
      const pricingRules = await pricingRepository.findByLaundryService(
        service._id.toString(),
      );

      const basePricing = pricingRules.find(
        (rule) => rule.pricingType === PricingType.BASE,
      );

      if (!basePricing) {
        throw new NotFoundError(
          `Base pricing not found for service "${service.name}".`,
        );
      }
   ...
   * Create Order
   */
  async create(userId: string, data: CreateOrderData): Promise<IOrder> {
    // 1. Validate user
   ...
   */
  async markReceivedAtFacility(orderId: string): Promise<IOrder> {
    this.validateObjectId(orderId, "order id");
    ...
    if (order.status !== OrderStatus.PICKED_UP) {
      throw new ValidationError(
        "Order must be PICKED_UP before it can be received at the facility.",
      );
    }
   ...
   */
  async startInspection(orderId: string): Promise<IOrder> {
    this.validateObjectId(orderId, "order id");
    ...
    if (order.status !== OrderStatus.RECEIVED_AT_FACILITY) {
      throw new ValidationError(
        "Order must be RECEIVED_AT_FACILITY before inspection can start.",
      );
    }
   ...
   *     PRICE_FINALIZED
   */
  async updatePricing(orderId: string, finalPrice: number): Promise<IOrder> {
    this.validateObjectId(orderId, "order id");
    ...
    if (order.status !== OrderStatus.INSPECTION_IN_PROGRESS) {
      throw new ValidationError(
        "Order must be in inspection before pricing can be finalized.",
      );
    }
   ...
   */
  async requestCustomerApproval(orderId: string): Promise<IOrder> {
    this.validateObjectId(orderId, "order id");
    ...
    if (order.status !== OrderStatus.PRICE_FINALIZED) {
      throw new ValidationError(
        "Order must be PRICE_FINALIZED before requesting customer approval.",
      );
    }
  ...
  async startQualityCheck(orderId: string): Promise<IOrder> {
    ...
    if (order.status !== OrderStatus.PROCESSING) {
      throw new ValidationError(
        "Order must be PROCESSING before quality check can start.",
      );
    }
   ...
   * READY_FOR_DELIVERY
   */
  async completeQualityCheck(orderId: string): Promise<IOrder> {
    this.validateObjectId(orderId, "order id");
    ...
    if (order.status !== OrderStatus.QUALITY_CHECK) {
      throw new ValidationError(
        "Order must be in QUALITY_CHECK before it can be marked ready for delivery.",
      );
    }
  ...
  async updatePaymentStatus(
    ...
    if (!order.isActive) {
      throw new ValidationError(
        "Cannot update payment status of an inactive order.",
      );
    }
    ...
    if (!allowedStatuses.includes(paymentStatus)) {
      throw new ValidationError(
        `Invalid payment status transition: ${order.paymentStatus} → ${paymentStatus}.`,
      );
    }
