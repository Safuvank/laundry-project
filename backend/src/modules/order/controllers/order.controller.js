...
class OrderController {
     ...
     */
    markReceivedAtFacility = asyncHandler(async (req, res) => {
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
        }
     ...
     */
    startInspection = asyncHandler(async (req, res) => {
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
        }
     ...
     */
    requestCustomerApproval = asyncHandler(async (req, res) => {
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
        }
     ...
     *       PROCESSING
     */
    approvePrice = asyncHandler(async (req, res) => {
        const userId = req.user.userId;
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
     ...
     *        ON_HOLD
     */
    rejectPrice = asyncHandler(async (req, res) => {
        const userId = req.user.userId;
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
     ...
     * QUALITY_CHECK
     */
    startQualityCheck = asyncHandler(async (req, res) => {
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
        }
     ...
     * READY_FOR_DELIVERY
     */
    completeQualityCheck = asyncHandler(async (req, res) => {
        const orderId = req.params.id;
        if (!orderId || Array.isArray(orderId)) {
            throw new ValidationError("Invalid order id.");
        }
