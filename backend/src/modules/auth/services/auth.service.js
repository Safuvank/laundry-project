...
export class AuthService {
    ...
    async register(data) {
        ...
        if (existingUser) {
            if (existingUser.accountStatus === "SUSPENDED") {
                throw new ValidationError("This account has been deactivated. Please restore your account or contact support.");
            }
    ...
    async login(email, password) {
        ...
        if (user.authProvider === "GOOGLE" || !user.password) {
            throw new UnauthorizedError("This account uses Google login. Please continue with Google.");
        }
    ...
    async googleCallback(code) {
        ...
        try {
            ...
            throw new UnauthorizedError("Unable to authenticate with Google. Please try again.");
        }
        if (!tokens.id_token) {
            throw new UnauthorizedError("Google authentication failed. ID token was not provided.");
        }
        /*
        ...
        try {
            ...
            throw new UnauthorizedError("Unable to verify your Google account. Please try again.");
        }
        if (!payload) {
            throw new UnauthorizedError("Unable to retrieve Google account information.");
        }
        /*
         * Google provider ID.
         *
        ...
        if (!payload.email_verified) {
            throw new UnauthorizedError("Your Google email address could not be verified.");
        }
        /*
        ...
        if (existingEmailUser) {
            if (existingEmailUser.authProvider === "LOCAL") {
                throw new ValidationError("An account with this email already exists. Please sign in using your email and password.");
            }
            /*
             * Unexpected provider state.
             */
            throw new ValidationError("An account with this email already exists. Please use the original sign-in method.");
        }
        /*
         * ------------------------------------------------------------------------
         * CASE 3
    ...
    async resetPassword(token, password) {
        ...
        const user = await authRepository.findUserById(passwordReset.userId.toString());
        if (!user) {
         ...
         */
        if (user.authProvider === "GOOGLE" || !user.password) {
            throw new ValidationError("This account uses Google login. Password reset is not available.");
        }
