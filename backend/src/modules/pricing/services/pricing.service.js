...
export class PricingService {
    ...
    async create(data) {
        ...
        /* ---------------------------------------------------------------------- */
        const laundryService = await laundryServiceRepository.findById(laundryServiceId);
        if (!laundryService) {
        ...
        /* ---------------------------------------------------------------------- */
        const existingRule = await pricingRepository.findByLaundryServiceAndType(laundryServiceId, data.pricingType);
        if (existingRule) {
            throw new ValidationError("Pricing rule already exists for this laundry service.");
        }
        /* ---------------------------------------------------------------------- */
        /* Create Pricing Rule                                                    */
        /* ---------------------------------------------------------------------- */
    ...
    async update(id, data) {
        ...
        /* Update Pricing Rule                                                    */
        /* ---------------------------------------------------------------------- */
        const updated = await pricingRepository.update(pricingRule._id.toString(), data);
        if (!updated) {
            throw new NotFoundError("Pricing rule not found.");
        }
        return updated;
    ...
    async activate(id) {
        const pricingRule = await this.getPricingRuleOrFail(id);
        const updated = await pricingRepository.activate(pricingRule._id.toString());
        if (!updated) {
            throw new NotFoundError("Pricing rule not found.");
        }
        return updated;
    ...
    async deactivate(id) {
        const pricingRule = await this.getPricingRuleOrFail(id);
        const updated = await pricingRepository.deactivate(pricingRule._id.toString());
        if (!updated) {
            throw new NotFoundError("Pricing rule not found.");
        }
        return updated;
