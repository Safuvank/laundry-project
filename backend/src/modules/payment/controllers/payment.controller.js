...
class PaymentController {
     ...
     */
    markPaymentSuccess = asyncHandler(async (req, res) => {
        const paymentId = req.params.id;
        if (!paymentId || Array.isArray(paymentId)) {
            throw new ValidationError("Invalid payment id.");
        }
        ...
        const payment = await paymentService.markPaymentSuccess(paymentId, transactionId);
        return res.status(200).json({
            success: true,
    ...
    markPaymentFailed = asyncHandler(async (req, res) => {
        ...
        const payment = await paymentService.markPaymentFailed(paymentId, failureReason);
        return res.status(200).json({
            success: true,
    ...
    refundPayment = asyncHandler(async (req, res) => {
        ...
        const payment = await paymentService.refundPayment(paymentId, refundId, refundReason);
        return res.status(200).json({
            success: true,
